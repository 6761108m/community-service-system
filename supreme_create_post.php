...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (in_array($imageFileType, $validExtensions) && $_FILES['image']['size'] <= 10000000) {
            ...
            echo "<script>alert('Invalid file type or size too large.'); window.location.href='supreme_create_post.html';</script>";
            exit();
        }
    }
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>alert('Post created successfully!'); window.location.href='supreme_feed.php';</script>";
    } else {
        // Error occurred
